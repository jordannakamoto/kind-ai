    ...
    <div className="w-full max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-2">
          {allTags.map((tag) => (
            <button
              ...
              className={`
                px-3 py-1 text-sm rounded-full transition duration-300 ease-in-out 
      ...
      <div className="grid grid-cols-3 grid-rows-2 gap-6">
        {filtered.map((mod) => (
          <div
            key={mod.id}
            className={`relative group overflow-hidden rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl ${
              sizeClass[mod.size]
            }`}
          >
            {/* Image */}
