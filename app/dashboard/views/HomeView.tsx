...
export default function UserCheckInConversation() {
  ...
  const varsRef = useRef({ userProfile: "", therapyModule: "", greeting: "", systemPrompt: "" });
  ...
  const conversation = useConversation({
    onMessage: (msg) => setAgentMessage(msg.text),
    onConnect: () => {
  ...
  useEffect(() => {
    if (!started) return;
    const animate = () => {
      const vol = conversation.isSpeaking ? conversation.getOutputVolume() : conversation.getInputVolume();
  ...
  const startConversation = async () => {
    ...
    const id = await conversation.startSession({ signedUrl, dynamicVariables: varsRef.current });
...
{/* Orb Wrapper with Fixed Size */}
<div
  className="relative flex items-center justify-center"
  style={{ width: "200px", height: "200px" }} // max expected size of orb
>
  <div
    ...
    {started && (
      <button className="flex items-center gap-2 hover:text-black">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 1v2m6.364 1.636l-1.414 1.414M21 12h-2m-1.636 6.364l-1.414-1.414M12 21v-2m-6.364-1.636l1.414-1.414M3 12h2m1.636-6.364l1.414 1.414" />
        </svg>
