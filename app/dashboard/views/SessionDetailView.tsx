...
export default function SessionDetailView({
  ...
  useEffect(() => {
    const fetchSession = async () => {
      const cacheKey = getCacheKey('session', sessionId);
      const cachedSession = getCachedData<Session>(cacheKey);
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      ...
      const cacheKey = getCacheKey('feedback', `${sessionId}_${transcriptHash}`);
      const cachedFeedback = getCachedData<typeof feedback>(cacheKey);
      
      if (cachedFeedback) {
        console.log('📦 Using cached feedback for:', sessionId);
